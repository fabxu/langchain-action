...
class SimpleCurrencyAgent:
    ...
    def _respond_irrelevant1(self, state: CurrencyState) -> CurrencyState:
        """生成不相关查询的响应"""
        exchange_data = state["exchange_data"]
...
def main():
    ...
    for i, query in enumerate(test_queries, 1): # enumerate()函数会返回一个索引和值,1 是起始索引值
        print(f"\n--- 测试 {i}: {query} ---")
...
# 这个应用只是说用llm 做了意图识别而已，但取from to 逻辑有缺陷，并且依赖字符匹配不太智能