...
assistant_role_name = "车展营销专家"
user_role_name = "汽车公司老板"
task = "整理出一个车展营销活动的策略"
...
# 使用新的提示模板方式
task_specifier_template = ChatPromptTemplate.from_messages([
    ("system", task_specifier_sys_msg.content),
    ("human", task_specifier_prompt)
])

# 创建模型实例
task_spec_model = ChatOpenAI(
    model="gpt-3.5-turbo",  # 注意：原代码中的"gpt-3.5"不是有效模型名称
    temperature=1.0,
    api_key=api_key
)

# 创建任务指定器链
task_spec_chain = task_specifier_template | task_spec_model

# 执行任务指定
specified_task_msg = task_spec_chain.invoke({
...
def get_sys_msgs(assistant_role_name: str, user_role_name: str, task: str):
    assistant_sys_msg = SystemMessage(content=assistant_inception_prompt.format(
        assistant_role_name=assistant_role_name,
        user_role_name=user_role_name,
        task=task
    ))

    user_sys_msg = SystemMessage(content=user_inception_prompt.format(
        assistant_role_name=assistant_role_name,
        user_role_name=user_role_name,
        task=task
    ))
...
assistant_agent = CAMELAgent(
    assistant_sys_msg, 
    ChatOpenAI(temperature=0.2, api_key=api_key)
)
